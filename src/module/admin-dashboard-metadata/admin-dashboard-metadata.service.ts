...
export class AdminDashboardMetadataService {
  ...
  async getDashboardData() {
    ...
    // 4. Real Revenue — ONLY from Lemon Squeezy subscriptions
    let totalRevenue = 0;
    let monthlyRevenue = 0;

    try {
      // Get all Lemon Squeezy subscription IDs from your database
      const appSubscriptions = await this.prisma.subscription.findMany({
        select: { lemonSubscriptionId: true },
        where: { lemonSubscriptionId: { not: null } },
      });

      for (const sub of appSubscriptions) {
        const subId = sub.lemonSubscriptionId;
      ...
      // Lemon totals are in cents — convert to dollars
      totalRevenue /= 100;
      monthlyRevenue /= 100;
    } catch (error) {
      this.logger.warn('Failed to fetch revenue from Lemon Squeezy', error);
      totalRevenue = 0;
      monthlyRevenue = 0;
    }
    ...
    const premiumPercentage =
      totalUsers > 0 ? Math.round((proSubscribers / totalUsers) * 100) : 0;
  ...
  async getAnalyticsData() {
    ...
    const avgStudyTimeMinutes =
      dailyActiveUsers > 0
        ? Math.round(
            (todaySessions._sum.durationSeconds || 0) / 60 / dailyActiveUsers,
          )
        : 0;
    ...
    const contentCompletion =
      totalPublishedLessons > 0 && totalUsers > 0
        ? Number(
            ((totalCompleted._sum.lessonsCompleted || 0) /
              (totalUsers * totalPublishedLessons)) *
              100,
          ).toFixed(1)
        : '0.0';
    ...
    const contentPerformance: ContentPerformanceItem[] = topLessons.map(
      (lesson) => ({
        lesson: `${lesson.skill || 'Unknown'} - ${lesson.task_id || 'Untitled'}`,
        views: lesson._count.practiceSessions,
      }),
    );
