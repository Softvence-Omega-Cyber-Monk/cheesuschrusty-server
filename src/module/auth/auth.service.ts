...
async register(dto: RegisterDto) {
  ...
  if (securitySettings) {
    ...
    this.logger.warn('SecuritySettings not found, using default password validation');
  ...
  const hashedPassword = await bcrypt.hash(dto.password, parseInt(process.env.SALT_ROUND || '10'));
...
async login(dto: LoginDto) {
  ...
  const sessionTimeoutMs = (securitySettings?.sessionTimeoutDays || 1) * 24 * 60 * 60 * 1000;
...
async refreshTokens(token: string) {
  try {
    ...
    return getTokens(this.jwtService, user.id, user.email, user.role, sessionTimeoutMs);
  } catch (error) {
...
async changePassword(email: string, dto: ChangePasswordDto) {
  ...
  const hashed = await bcrypt.hash(dto.newPassword, parseInt(process.env.SALT_ROUND || '10'));
  await this.prisma.user.update({
    where: { email },
...
async resetPassword(dto: ResetPasswordDto) {
  ...
  if (!verified) {
    throw new BadRequestException('OTP not verified. Please verify the code first.');
  }
  ...
  const hashed = await bcrypt.hash(dto.password, parseInt(process.env.SALT_ROUND || '10'));
  await this.prisma.user.update({
    where: { email: dto.email },
    data: { password: hashed },
  });
