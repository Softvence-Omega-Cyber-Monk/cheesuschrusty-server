...
export async function getTokens(
  ...
  const accessTokenExpiresIn: JwtExpiresIn = sessionTimeoutMs
    ? Math.floor(sessionTimeoutMs / 1000)
    : ((process.env.ACCESS_TOKEN_EXPIREIN as JwtExpiresIn | undefined) ?? '1h');

  const refreshTokenExpiresIn: JwtExpiresIn =
    (process.env.REFRESH_TOKEN_EXPIREIN as JwtExpiresIn | undefined) ?? '7d';
