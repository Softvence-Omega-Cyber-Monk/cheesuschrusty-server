...
export class PromptController {
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - Empty or invalid prompt text',
  })
  async updateMasterPromptQuestions(
    ...
    const result = await this.promptService.updateMasterPromptQuestions({
      promptText: rawText,
    });
    return sendResponse(res, {
  ...
  @ApiOperation({
    ...
    description:
      'Set or update the master prompt used for providing feedback. Send the complete prompt as plain text in the request body. All characters (quotes, brackets, newlines, symbols) are preserved exactly as sent. Content-Type must be text/plain.',
  })
  @ApiResponse({
    status: 200,
  ...
  async updateMasterPromptFeedback(
    ...
    const result = await this.promptService.updateMasterPromptFeedback({
      promptText: rawText,
    });
    return sendResponse(res, {
      statusCode: HttpStatus.OK,
  ...
  @ApiOperation({
    ...
    description:
      'Retrieve the complete current master prompt used for providing feedback. Returns the full prompt text exactly as it was stored, with all formatting and special characters preserved.',
  })
  @ApiResponse({
    status: 200,
