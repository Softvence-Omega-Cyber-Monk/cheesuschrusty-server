...
export class ExternalServicesHealthIndicator extends HealthIndicator {
  ...
  async checkService(key: string, provider: CredentialProvider): Promise<HealthIndicatorResult> {
    try {
      ...
      if (!credentials || Object.keys(credentials).length === 0) {
        return this.getStatus(key, true, { status: 'unconfigured', message: 'No credentials found' });
      }
      ...
      if (providerStr === 'STRIPE') {
        ...
        await axios.get('https://api.stripe.com', { timeout: 3000, validateStatus: () => true });
      } else if (providerStr === 'CLOUDINARY') {
        await axios.get('https://cloudinary.com', { timeout: 3000, validateStatus: () => true });
      }
