...
} from './dto/flashcard.response.dto';
import {
  CreateCardDto,
  CreateCategoryDto,
  UpdateCardDto,
...
export class FlashcardService {
  ...
  async createCard(dto: CreateCardDto) {
    ...
    this.logger.log(
      `Creating new card in category ${dto.categoryId}: ${dto.frontText}`,
    );
  ...
  async getDashboardOverview(
    userId: string,
  ): Promise<FlashcardOverviewResponseDto> {
    ...
    const categorySummaries = categories.map((category) => {
      ...
      const categoryProgress = allUserProgress.filter((p) =>
        categoryCardIds.has(p.cardId),
      );

      const dueCount = categoryProgress.filter(
        (p) => p.nextReview && p.nextReview <= today,
      ).length;
      const masteredCount = categoryProgress.filter(
        (p) => p.repetitions >= this.MASTERY_THRESHOLD,
      ).length;
      const isActiveSession = activeSessions.some(
        (s) => s.categoryName === category.title,
      );
  ...
  async startSession(
    ...
    const category = await this.prisma.flashcardCategory.findUnique({
      where: { id: dto.categoryId },
      include: {
        cards: { select: { id: true, frontText: true, backText: true } },
      },
    });
    ...
    if (cardIdsForSession.length < SESSION_LIMIT) {
      ...
      const newIds = categoryCardIds
        .filter((id) => !studiedIds.has(id))
        .slice(0, SESSION_LIMIT - cardIdsForSession.length);
  ...
  async pauseSession(
    ...
    this.logger.log(
      `Session ${sessionId} paused – saved time: ${currentTimeSeconds}s`,
    );
  }
  ...
  async gradeCard(
    ...
    if (!sessionFinished) {
      ...
      const nextCard = await this.prisma.card.findUnique({
        where: { id: nextId },
      });
