...
class CardInSessionDto {
  ...
  @ApiProperty({
    description: 'The text displayed on the front of the card.',
    example: 'Hash Map',
  })
  frontText: string;

  @ApiProperty({
    description: 'The text displayed on the back of the card.',
    example: 'A data structure using a hash function.',
  })
  backText: string;

  @ApiProperty({
    description:
      'The current calculated interval (days) for the next review based on SRS.',
    example: 6,
  })
...
class SessionScoresDto {
  @ApiProperty({
    description:
      'Total number of cards graded correctly (grade >= 2) in this session.',
    example: 5,
  })
  correctCount: number;
  ...
  @ApiProperty({
    description:
      'Total number of cards graded incorrectly (grade < 2) in this session.',
    example: 2,
  })
...
export class StartSessionResponseDto {
  ...
  @ApiProperty({
    description: 'The current status of the session (ACTIVE, PAUSED).',
    example: 'ACTIVE',
  })
  status: 'ACTIVE' | 'PAUSED';

  @ApiProperty({
    description: 'The number of card IDs left in the review queue.',
    example: 15,
  })
  cardsRemaining: number;

  @ApiProperty({
    type: SessionScoresDto,
    description: 'The current scores for the session.',
  })
  scores: SessionScoresDto;

  @ApiProperty({
    type: CardInSessionDto,
    description: 'The card currently presented to the user.',
  })
...
export class GradeCardResponseDto {
  @ApiProperty({
    type: SessionScoresDto,
    description: 'The updated scores for the session.',
  })
  scores: SessionScoresDto; // Renamed from updatedScores

  @ApiProperty({
    description: 'True if the session finished with this grade submission.',
    example: false,
  })
  ...
  @ApiProperty({
    ...
    nullable: true,
    description:
      'The next card to be presented. Null if sessionFinished is true.',
  })
